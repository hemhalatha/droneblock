...
class SafetyRule:
    ...
    """
    def __init__(self, condition: Callable[['DroneState'], bool], action: Action, priority: int = 0) -> None:
...
class SafetyManager:
    ...
    def _check_rules(self, data: Any = None) -> None:
        ...
        for rule in self.rules:
            if not rule.triggered and rule.condition(state):
                log.warning(f"SAFETY VIOLATION DETECTED (Priority {rule.priority}): {rule.action}")
                rule.triggered = True
                
                # Interrupt the currently executing mission step
                if self.drone.current_action:
                    log.info(f"Aborting current action '{self.drone.current_action}' due to safety constraint.")
                    self.drone.current_action.abort()
                
                # Immediately initiate the safety recovery action
                log.info(f"Starting safety action: {rule.action}")
                rule.action.start()
