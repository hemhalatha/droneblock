"""
DroneBlock PyMavlink Connector Module.
...
from ..core.exceptions import ConnectionError as DroneblockConnectionError
...
class PymavlinkConnector(BaseConnector):
    ...
    def connect(self) -> None:
        ...
        """
        log.info("Opening MAVLink connection at %s", self.url)
        ...
        if not msg:
            raise DroneblockConnectionError(
                f"Failed to connect to vehicle at {self.url} (Heartbeat timeout)"
            )
    ...
    def _recv_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                log.error("MAVLink receive error: %s", e)
    ...
    def set_mode(self, mode_name: str) -> None:
        ...
        log.debug("Mode change sent: %s (%s)", mode_name, mode_id)
